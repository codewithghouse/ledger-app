...
import { 
  FileBox, Plus, Loader2, Trash2, Save, ShoppingCart, User
} from "lucide-react";
...
const SalesOrdersPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await ordersApi.create({ ...form, entity_name: customer?.name || "N/A", total_amount: total, date: new Date().toISOString() });
      toast({ title: "✅ Order Created", description: "Sales order recorded successfully" });
      setForm({
        order_no: `SO-${Date.now().toString().slice(-6)}`,
        entity_id: "",
        items: [{ product_id: "", qty: 1, price: 0 }],
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <FileBox className="w-6 h-6 text-primary" /> Sales Orders
          </h1>
          <p className="text-sm text-muted-foreground">Manage customer quotes and pending orders</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <form onSubmit={handleSubmit} className="lg:col-span-1 glass-card p-6 rounded-2xl space-y-4 h-fit">
          <h3 className="text-sm font-bold uppercase tracking-wider">Create New Order</h3>
          ...
          <button type="submit" disabled={submitting} className="w-full py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm flex items-center justify-center gap-2 hover:opacity-90 transition-all shadow-lg shadow-primary/20">
            {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save Order
          </button>
        </form>

        <div className="lg:col-span-2 glass-card rounded-2xl overflow-hidden">
          <div className="p-4 border-b border-border/50 bg-muted/10">
            <h3 className="text-xs font-bold uppercase text-muted-foreground">Pending Sales Orders</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={4} className="p-12 text-center text-muted-foreground"><Loader2 className="w-8 h-8 animate-spin mx-auto mb-2 text-primary" /> Loading orders...</td></tr>
                ) : data.length === 0 ? (
                  <tr><td colSpan={4} className="p-12 text-center text-muted-foreground">No orders found.</td></tr>
                ) : data.map((d, i) => (
