...
import { 
  FileBox, Plus, Loader2, Trash2, Save, ShoppingBag, Truck
} from "lucide-react";
...
const PurchaseOrdersPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const vendor = vendors.find(v => v.id === form.entity_id);
      await ordersApi.create({ ...form, entity_name: vendor?.name || "N/A", total_amount: total, date: new Date().toISOString() });
      toast({ title: "✅ Order Created", description: "Purchase order recorded successfully" });
      setForm({
        order_no: `PO-${Date.now().toString().slice(-6)}`,
        entity_id: "",
        items: [{ product_id: "", qty: 1, price: 0 }],
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <Truck className="w-6 h-6 text-primary" /> Purchase Orders
          </h1>
          <p className="text-sm text-muted-foreground">Manage inventory requests and vendor orders</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <form onSubmit={handleSubmit} className="lg:col-span-1 glass-card p-6 rounded-2xl space-y-4 h-fit">
          <h3 className="text-sm font-bold uppercase tracking-wider">New Purchase Order</h3>
          <div className="space-y-2">
            <label className="text-xs font-bold text-muted-foreground uppercase">Order #</label>
            <input readOnly value={form.order_no} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-xs font-mono" />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <label className="text-xs font-bold text-muted-foreground uppercase">Items to Order</label>
          ...
          <button type="submit" disabled={submitting} className="w-full py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm flex items-center justify-center gap-2 hover:opacity-90 transition-all shadow-lg shadow-primary/20">
            {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Issue Order
          </button>
        </form>

        <div className="lg:col-span-2 glass-card rounded-2xl overflow-hidden">
          <div className="p-4 border-b border-border/50 bg-muted/10">
            <h3 className="text-xs font-bold uppercase text-muted-foreground">Pending Vendor Orders</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={4} className="p-12 text-center text-muted-foreground"><Loader2 className="w-8 h-8 animate-spin mx-auto mb-2 text-primary" /> Loading ledger...</td></tr>
                ) : data.length === 0 ? (
                  <tr><td colSpan={4} className="p-12 text-center text-muted-foreground">No orders tracked.</td></tr>
                ) : data.map((d, i) => (
                  <tr key={i} className="border-b border-border/30 hover:bg-muted/50 transition-colors">
                    ...
                    <td className="p-4 font-mono font-bold text-primary">{d.order_no}</td>
